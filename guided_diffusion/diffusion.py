...
class Diffusion(object):
    ...
    def simplified_ddnm_plus(self, model, cls_fn):
            ...
            with open(txt_file,'r') as file:
                for line in file:
                    ...
                    if len(parts) == 2:
                        ...
                        class_to_npy[class_index] = f'npy/npy_test_mask/{file_name}.npy'
        ...
        txt_file = 'exp/txt/murals_test_2k.txt'
        ...
        if args.deg =='colorization':
            ...
            #mask = torch.from_numpy(mask_npy).to(self.device) #确认mask
            A = lambda z: z*mask
            Ap = A
            Ap = A
            '''
        ...
        for x_orig, classes in pbar:
            ...
            mask_npy=np.load(class_to_npy[npy_class])
                #mural_name = class_to_name[npy_class]
                #print(mask_loaded.shape)
            mask = torch.from_numpy(mask_npy).to(self.device)
            A = lambda z: z*mask
            Ap = A
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        for x_orig, classes in pbar:
            ...
            mask_npy=np.load(class_to_npy[npy_class])
                #mural_name = class_to_name[npy_class]
                #print(mask_loaded.shape)
            mask = torch.from_numpy(mask_npy).to(self.device).reshape(-1) #确认mask
            ...
            for i in range(len(Apy)):
                tvu.save_image(
                    inverse_data_transform(config, Apy[i]),
                    os.path.join(self.args.image_folder, mural_name)
                )
                tvu.save_image(
                    inverse_data_transform(config, x_orig[i]),
                    os.path.join(self.args.image_folder, mural_name)
            ...
            for j in range(x[0].size(0)):
                tvu.save_image(
                    x[0][j], os.path.join(self.args.image_folder, mural_name)
                )
