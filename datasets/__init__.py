...
#             )
    ...
    elif config.data.dataset == "MURALS":
        if config.data.subset_2k:
            from datasets.imagenet_subset import ImageDataset
            test_dataset =ImageDataset(os.path.join(args.exp,'datasets', 'murals' ,'0'),#, 
                     os.path.join(args.exp, 'txt/murals_test_2k.txt'),
                     image_size=config.data.image_size,
                     normalize=False) 
        if config.data.sketch:
            dataset = ImageDataset(os.path.join(args.exp, 'datasets', 'sketch', '0'),#
                     os.path.join(args.exp, 'txt/murals_test_2k.txt'),
                     image_size=config.data.image_size,
                     normalize=False)
